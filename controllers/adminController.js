...
const randomSchema=require("../models/randomMedia");
...
exports.uploadFile=(req,res)=>{
  ...
  upload(req,res,(err)=>{
      ...
      let newproduct=new Data({"productPath":req.file.path,"productCategory":req.category._id,"productName":req.body.productName,"mediaId":req.body.mediaId,"description":req.body.description});
...
exports.getAllFiles=(req,res)=>{
    ...
    setInterval(()=>{
     Data.find().populate('productCategory').exec((err,count)=>{
       let AudiosArray=count.filter((item)=>item.productCategory.categoryName=="Audios");
       let VediosArray=count.filter((item)=>item.productCategory.categoryName=="Vedios");
       let ImagesArray=count.filter((item)=>item.productCategory.categoryName=="Images");

       let randomAudio=Math.floor(Math.random()*AudiosArray.length);
       let randomVedio=Math.floor(Math.random()*VediosArray.length);
       let randomImages=Math.floor(Math.random()*ImagesArray.length);
       console.log(randomAudio,randomVedio,randomImages)

        let saveRandom={"AudioId":AudiosArray[randomAudio]._id,"VedioId":VediosArray[randomVedio]._id,"ImageId":ImagesArray[randomImages]._id};
        
        randomMedia.updateOne(saveRandom).exec((err,savedata)=>{
            if(err) throw err
