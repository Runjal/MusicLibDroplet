...
const randomMedia = require("../models/randomMedia");
...
exports.getBookmarkData=(req,res)=>{
     ...
     let uniquser=[...new Set(mainar)]
       ...
       console.log("Uniquser is",uniquser)
        ...
        //Data.find({_id: r.Bookmark}).populate("productCategory").exec((error,found)=>{
            Data.findOne({_id: req.body.productId}).populate("productCategory").exec((error,found)=>{
                if(error)
                {
                    //console.log(error)
                    return res.status(422).json({
    ...
    let val={"prouctId":found._id,"productName":found.productName,"Category":found.productCategory._id,"CategoryName":found.productCategory.categoryName=="Vedios"? 1 : 0};           
...
exports.getIndexPageData=(req,res)=>{
    console.log("in image")
    randomMedia.find().populate("ImageId").populate("VedioId").populate("AudioId").exec((err,Imgedata)=>{
        let indexpageData=[
            {
                Vedio:{
                _id:Imgedata[0].VedioId._id,
                productName:Imgedata[0].VedioId.productName,
                productPath:Imgedata[0].VedioId.productPath,
                description:Imgedata[0].VedioId.description || null
                
                },
               
                Audio:{
                    _id:Imgedata[0].AudioId._id,
                    productName:Imgedata[0].AudioId.productName,
                    productPath:Imgedata[0].AudioId.productPath,
                    description:Imgedata[0].AudioId.description || null
                    
                    },
                    Image:{
                        _id:Imgedata[0].ImageId._id,
                        productName:Imgedata[0].ImageId.productName,
                        productPath:Imgedata[0].ImageId.productPath,
                        description:Imgedata[0].ImageId.description || null
                        
                        },
        }
        ]

        return res.status(200).json(indexpageData)
        
})

}

exports.resolvemedia=(req,res)=>{
    console.log("Reched reolve media")
    let resoslvedpth=path.resolve(req.query.id)

    res.sendFile(resoslvedpth)
}
